...
const sfs = await import("../forge/securefs.js")
...
console.log("== concurrent writers: two tools, one file, no torn content ==")
{
  ...
  const script = `import { makeToolContext } from ${JSON.stringify(new URL("../forge/tools.js", import.meta.url).href)}; const t = makeToolContext({ cwd: ${JSON.stringify(PROJ)}, root: ${JSON.stringify(PROJ)} }); for (let i=0;i<30;i++) await t.exec("write_file", { path: "multi.txt", content: process.argv[2].repeat(50000) }); console.log("done")`
