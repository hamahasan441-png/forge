...
const FORGE = path.join(HERE, "../forge")
...
console.log("== completed PLAN files are gone ==")
{
  ...
  ok("has open todos", open >= 8, open)
  ok("skill forge leftover", /Research → implement a new skill/.test(todo))
  ok("STALE leftover", /STALE-by-age/.test(todo))
  ok("land main leftover", /Land v38/.test(todo))
