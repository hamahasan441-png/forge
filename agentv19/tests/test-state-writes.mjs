...
const { writeStateFile } = await import("../forge/securefs.js")
...
console.log("== crash between temp write and rename (real process kill) ==")
{
  ...
  const script = `
    import fs from "node:fs"
    const { writeStateFile } = await import(${JSON.stringify(new URL("../forge/securefs.js", import.meta.url).href)})
...
{
  const { saveSession, loadSession } = await import("../forge/sessions.js")
  const { saveConfig } = await import("../forge/config.js")
  const { recordHealth, readHealth } = await import("../forge/health.js")
...
{
  const { openTask, readTask, TASK_STATUS } = await import("../forge/taskstate.js")
