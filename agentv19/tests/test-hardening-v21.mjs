...
const { modelMayRun, classifyCommand, protectedDestinationReason } = await import("../forge/shellguard.js")
const { redact } = await import("../forge/secrets.js")
const { sanitizeProjectConfig, defaultConfig } = await import("../forge/config.js")
const { childEnv } = await import("../forge/childenv.js")
...
console.log("== CF-1 project config cannot escalate (pre-fix: unrestricted deepMerge) ==")
{
  ...
  process.chdir(PROJ)
  try {
    const { loadConfig } = await import("../forge/config.js")
...
console.log("== CE-1 child environment for MCP/LSP (pre-fix: full process.env inherited) ==")
{
  ...
  const mcpSrc = fs.readFileSync(new URL("../forge/mcp.js", import.meta.url), "utf8")
  const lspSrc = fs.readFileSync(new URL("../forge/lsp.js", import.meta.url), "utf8")
...
{
  const tools = fs.readFileSync(new URL("../forge/tools.js", import.meta.url), "utf8")
  ...
  const cp = fs.readFileSync(new URL("../forge/checkpoint.js", import.meta.url), "utf8")
