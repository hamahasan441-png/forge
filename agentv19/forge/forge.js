...
// Flags that never take a value. Without this list `forge agent --deep "task"`
// swallowed the task as the VALUE of --deep and died with a usage error, and
// `forge chat --continue` / `--pick` / `--plan` broke the same way whenever a
// positional followed them.
const BOOLEAN_FLAGS = new Set(["deep", "pick", "plan", "continue", "free", "tools", "all", "json", "no-color", "help", "version", "h", "v"])
// Short aliases documented in the README / help text (-m "msg", -v, -h).
// The old parser treated "-m" as a positional, so `forge chat -m "hi"` sent
// the literal text "-m hi" to the model.
const SHORT_FLAGS = { m: "m", v: "version", h: "help" }
...
export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith("--")) {
      const eq = a.indexOf("=")
      if (eq !== -1) { flags[a.slice(2, eq)] = a.slice(eq + 1); continue } // --key=value
      const key = a.slice(2)
      const next = argv[i + 1]
    ...
    if (/^-[a-zA-Z]$/.test(a) && SHORT_FLAGS[a[1]]) {
      const key = SHORT_FLAGS[a[1]]
      const next = argv[i + 1]
      if (!BOOLEAN_FLAGS.has(key) && next !== undefined && !next.startsWith("-")) { flags[key] = next; i++ }
      else flags[key] = true
      continue
  ...
  // --plan "task" used to work because the task landed in flags.plan — keep
  // that spelling working by promoting a string value back to a positional.
  if (typeof flags.plan === "string") { positional.push(flags.plan); flags.plan = true }
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "doctor": {
      ...
      console.log()
      ok("doctor done — fix fails with: forge config set providers.<name>.apiKey <KEY>")
      return
    }
    case "use": {
...
// run only when executed as the CLI — importing this module (tests) must not
// start the program
const isMain = (() => {
