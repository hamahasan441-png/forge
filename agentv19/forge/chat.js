    ...
    switch (cmd) {
      ...
      case "profile": {
        ...
        if (arg2 && valid.includes(arg2)) {
          ...
          saveConfig(config)
          // deep follows the profile: `deep` turns it on, `fast`/`balanced`
          // turn it off (unless --deep was passed on the CLI), `auto` leaves
          // the per-message auto-detection in charge. v20 only ever switched
          // it ON, so `/profile deep` → `/profile fast` kept deep mode.
          if (arg2 === "deep") deep = true
          else if (!deepFlag && !config.chat?.deep) deep = false
          ok(`effort profile → ${bold(arg2)}${arg2 === "auto" ? dim(" (complex tasks automatically get deep thinking)") : ""}${deep && arg2 !== "deep" ? dim(" (deep still on via /deep or --deep)") : ""}`)
        } else if (arg2) {
