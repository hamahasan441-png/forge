...
const BASE_HEADERS = { "user-agent": "forge-agent/20.0.1" }
...
async function* streamOpenAI(opts, base) {
  ...
  // v20.0.1: the streaming path never sent `tools` (the Anthropic stream and
  // the non-streaming chatOnce did) — so in `forge chat` the model could not
  // call a single tool on any OpenAI-protocol provider.
