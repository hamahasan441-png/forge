...
function realPathOf(abs, depth = 0) {
  try {
    return fs.realpathSync(abs)
  } catch {}
  if (depth > 32) return abs
  // Not fully resolvable: the tail does not exist yet, or a component is a
  // DANGLING symlink. realpath() fails on both, and the v20 fallback
  // (realpath of the nearest existing ancestor + basename) kept a dangling
  // symlink as a plain name — `ln -s ~/.ssh/authorized_keys ak` followed by
  // write_file("ak") therefore looked like an in-project write while the OS
  // created the file outside. Resolve component by component instead.
  const parts = abs.split(path.sep).filter(Boolean)
  let cur = path.parse(abs).root
  for (let i = 0; i < parts.length; i++) {
    const next = path.join(cur, parts[i])
    let st
    try { st = fs.lstatSync(next) } catch { return path.join(next, ...parts.slice(i + 1)) } // missing tail: plain names from here
    if (st.isSymbolicLink()) {
      let target
      try { target = fs.readlinkSync(next) } catch { return path.join(next, ...parts.slice(i + 1)) }
      return realPathOf(path.join(path.resolve(cur, target), ...parts.slice(i + 1)), depth + 1)
    }
    cur = next
  }
  return cur
...
export function safePath(ctx, p, { write = false } = {}) {
  ...
  // v20.0.1: compare the target's REAL path against the project root's REAL
  // path. v20 compared it against the logical root, so any project reached
  // through a symlink (macOS /tmp → /private/tmp, ~/projects → /mnt/…) had
  // EVERY write rejected as "escapes the project directory". A target is
  // considered inside when either its logical or real form is inside the
  // root's logical or real form — but a symlink that points OUT of the
  // project is still caught because its real path lands outside both.
  const rootLogical = ctx.root ?? ctx.cwd
  const rootReal = realPathOf(rootLogical)
  const inRoot = (target) => insideDir(target, rootLogical) || insideDir(target, rootReal)
  if (write) {
    if (!ctx.allowOutsideProject && !inRoot(real)) {
      return {
        ok: false, abs, real,
        error: `ERROR: write target escapes the project directory (${path.relative(rootReal, real).slice(0, 60)}) — keep changes inside ${rootLogical}, or set tools.allowOutsideProject: true in config to allow it`,
      }
    }
    return { ok: true, abs, real }
  }
  // read policy
  if (!inRoot(real) || !inRoot(abs)) {
...
function edit_file(ctx, args) {
  ...
  const oldS = args.old ?? ""
  const newS = args.new ?? ""
  if (oldS === "") return "ERROR: old must be a non-empty string (use write_file to create or fully replace a file)"
  ...
  const out = args.replace_all ? src.split(oldS).join(newS) : replaceOnceLiteral(src, oldS, newS)
  fs.writeFileSync(p, out)
  return `OK edited ${p}`
}

/** Literal single replacement. `String.replace(str, str)` interprets `$&`,
 *  `$1`, `$$`, `$'` in the replacement — so an edit whose new text contained
 *  a template literal like `\`$${amount}\`` silently lost a `$`, and `"$&"`
 *  re-inserted the matched text (v20 corrupted real source files this way). */
function replaceOnceLiteral(src, oldS, newS) {
  const i = src.indexOf(oldS)
  if (i === -1) return src
  return src.slice(0, i) + newS + src.slice(i + oldS.length)
}
...
async function fetch_url(ctx, args) {
  ...
  try {
    // v20.0.1: redirects are followed MANUALLY so every hop passes the SSRF
    // guard. `redirect: "follow"` validated only the first URL — a public page
    // answering 302 → http://127.0.0.1/… or the metadata IP went straight
    // through (verified against a local listener).
    const headers = { "user-agent": "forge-agent/20.0.1", accept: "text/*,application/json;q=0.9,*/*;q=0.5" }
    const deadline = AbortSignal.timeout(15000)
    let cur = url
    let res = null
    for (let hop = 0; hop < 6; hop++) {
      res = await fetch(cur, { headers, redirect: "manual", signal: deadline })
      const loc = res.headers.get("location")
      if (!(res.status >= 300 && res.status < 400 && loc)) break
      let next
      try { next = new URL(loc, cur).toString() } catch { return `ERROR: malformed redirect target from ${cur}` }
      try { await res.arrayBuffer() } catch {}
      if (!/^https?:$/i.test(new URL(next).protocol)) return `BLOCKED (SSRF guard): redirect to a non-http(s) URL (${next.slice(0, 80)})`
      const v2 = await assertFetchableUrl(next, { allowPrivate: !!ctx.fetchPrivateUrls })
      if (!v2.ok) return `BLOCKED (SSRF guard): ${cur} redirected to ${next.slice(0, 80)} — ${v2.reason}`
      cur = next
      res = null
    }
    if (!res) return `ERROR: too many redirects for ${url}`
    if (!res.ok) return `ERROR: HTTP ${res.status} for ${cur}`
    ...
    return cap(`URL: ${cur}${cur !== url ? ` (redirected from ${url})` : ""}\nContent-Type: ${ct}\n\n${text}`, ctx.maxToolOutput)
  } catch (e) {
...
async function web_search(ctx, args) {
  ...
  if (ctx.searchUrl) {
    ...
    tried.push(url)
    try {
      const res = await fetch(url, { headers: { "user-agent": "forge-agent/20.0.1", accept: "application/json,text/html;q=0.8" }, signal: AbortSignal.timeout(12000) })
      if (res.ok) {
        const ct = res.headers.get("content-type") || ""
...
function multi_edit(ctx, args) {
  ...
  // Every `old` is matched against the ORIGINAL file, so edits must not
  // depend on each other (edit 2 cannot target text produced by edit 1).
  const seen = new Set()
  for (let i = 0; i < edits.length; i++) {
    const e = edits[i]
    if (!e || typeof e !== "object") return `ERROR: edit ${i + 1}: must be an object {old, new} (no changes applied)`
    const o = e.old ?? ""
    if (o === "") return `ERROR: edit ${i + 1}: old must be a non-empty string (no changes applied)`
    if (!src.includes(o)) {
      const producedEarlier = edits.slice(0, i).some((prev) => typeof prev?.new === "string" && prev.new.includes(o))
      return `ERROR: edit ${i + 1}: old string not found in file (no changes applied)${producedEarlier ? " — all edits match against the ORIGINAL text; do not chain an edit on the output of a previous edit, merge them into one" : ""}`
    }
    if (!e.replace_all && src.indexOf(o) !== src.lastIndexOf(o)) {
      return `ERROR: edit ${i + 1}: old string appears multiple times — add context or set replace_all (no changes applied)`
    }
    if (seen.has(o)) return `ERROR: edit ${i + 1}: duplicate old string (same text as an earlier edit) — only one edit per location is possible (no changes applied)`
    seen.add(o)
  }
  let out = src
  let applied = 0
  for (const e of edits) {
    ...
    else { out = replaceOnceLiteral(out, e.old, e.new ?? ""); applied++ }
  }
  snapshotBefore([p], ctx.cwd) // v16: auto-checkpoint
  fs.writeFileSync(p, out)
...
async function delegate(ctx, args) {
  ...
  try {
    ...
    // the timer MUST be cleared once the sub-agent returns: in v20 it kept the
    // event loop alive, so every `forge agent` run that delegated hung for
    // delegateTimeoutSec (180 s) after printing its final answer
    let timerId = null
    const timer = new Promise((resolve) => {
      timerId = setTimeout(() => { timedOut.v = true; resolve(null) }, timeoutMs)
    })
    let summary
    try { summary = await Promise.race([work, timer]) } finally { if (timerId) clearTimeout(timerId) }
