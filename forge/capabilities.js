...
export const BUILTIN_CAPABILITIES = [
  ...
  {
    name: "process",
    description: "Background process manager: spawn/poll/status/kill long-running commands (dev servers, watchers).",
    capabilities: [CAPABILITY.COMMAND_EXECUTION, "process_supervision", "runtime_observation"],
    klass: CLASS.EXECUTE, classes: [CLASS.EXECUTE, CLASS.READ],
    risk: RISK.MEDIUM, read_only: true, reversible: true, parallel_safe: false,
    requires_confirmation: false, requires_network: false, requires_filesystem: false,
    timeout: 15, cost: C(150, 500, 4000), verification_required: false, idempotent: false,
    preferred_for: ["keep a dev server alive while browser-testing the app you just built", "watch a long build/train and collect output incrementally", "launch, observe and restart project runtimes (§28)"],
    avoid_when: ["the command finishes in seconds (bash is simpler and already capped)", "you only need one-shot output"],
    mutates: [],
  },
  {
    name: "repl",
    description: "Persistent Node REPL session: state survives across calls for iterative analysis.",
    capabilities: ["interactive_evaluation", CAPABILITY.COMMAND_EXECUTION],
    klass: CLASS.EXECUTE, classes: [CLASS.EXECUTE],
    risk: RISK.MEDIUM, read_only: true, reversible: true, parallel_safe: false,
    requires_confirmation: false, requires_network: false, requires_filesystem: false,
    timeout: 20, cost: C(200, 800, 8000, { cpu: "medium" }), verification_required: false, idempotent: false,
    preferred_for: ["iterative data analysis with loaded state", "trying snippets without re-spawning a process", "inspecting values across several steps"],
    avoid_when: ["one-shot command output (bash)", "the analysis fits in a single expression"],
    mutates: [],
  },
  {
    name: "semantic_search",
    description: "Meaning-ranked code search over project chunks (BM25 + optional embeddings rerank).",
    capabilities: [CAPABILITY.CONTENT_SEARCH, CAPABILITY.SYMBOL_LOOKUP, CAPABILITY.FILE_DISCOVERY],
    klass: CLASS.READ, classes: [CLASS.READ],
    risk: RISK.LOW, read_only: true, reversible: true, parallel_safe: true,
    requires_confirmation: false, requires_network: false, requires_filesystem: true,
    timeout: 30, cost: C(250, 700, 8000, { cpu: "medium" }), verification_required: false, idempotent: true,
    preferred_for: ["find where a concept is handled when grep finds no literal match", "locate code by natural-language intent", "verifiers locating relevant code without mutating anything"],
    avoid_when: ["the exact symbol/text is known (grep_files is cheaper)", "the file is already known"],
    mutates: [],
  },
  {
    name: "runtime",
    description: "Runtime Intelligence: evidence-based project discovery, discovered-command launch, health probes, runtime claims, crash reconcile.",
    capabilities: [CAPABILITY.COMMAND_EXECUTION, "runtime_observation", "runtime_evidence", "project_discovery"],
    klass: CLASS.EXECUTE, classes: [CLASS.EXECUTE, CLASS.READ],
    risk: RISK.MEDIUM, read_only: true, reversible: true, parallel_safe: false,
    requires_confirmation: false, requires_network: true, requires_filesystem: true,
    timeout: 20, cost: C(120, 600, 6000), verification_required: false, idempotent: false,
    preferred_for: ["discover how this project actually runs before launching anything", "launch the discovered dev/build script and keep it alive", "prove 'server started' with a real health probe (verification-grade evidence)", "reconcile forge-owned processes after a crash without touching unrelated ones"],
    avoid_when: ["arbitrary one-shot commands (bash)", "raw process control without discovery (process tool)"],
    mutates: [],
  },
  {
    name: "kg_query",
    description: "Deterministic queries over the project knowledge graph: dependents, blast radius, covering tests, symbol locate, language, recent changes, plus the understand-anything knowledge graph when one was built.",
    capabilities: [CAPABILITY.CONTENT_SEARCH, CAPABILITY.SYMBOL_LOOKUP, CAPABILITY.FILE_DISCOVERY, CAPABILITY.DIRECTORY_LISTING],
    klass: CLASS.READ, classes: [CLASS.READ],
    risk: RISK.LOW, read_only: true, reversible: true, parallel_safe: true,
    requires_confirmation: false, requires_network: false, requires_filesystem: true,
    timeout: 30, cost: C(200, 600, 6000, { cpu: "medium" }), verification_required: false, idempotent: true,
    preferred_for: ["navigate architecture before editing (who imports this, what breaks)", "find the covering tests for a file before changing it", "read the understand-anything knowledge graph without re-deriving it"],
    avoid_when: ["the exact text is known (grep_files is cheaper)", "the world model is unavailable and the answer cannot wait"],
    mutates: [],
  },
  {
    name: "plan_whatif",
    description: "Deterministic what-if simulation of a plan (or a plan change) through the predictive risk engine: risk ladder, success estimate, factor breakdown, critical path, SPOF, real failure lessons, base-vs-mutated delta. Computes, never executes.",
    capabilities: ["plan_simulation", "risk_assessment"],
    klass: CLASS.READ, classes: [CLASS.READ],
    risk: RISK.LOW, read_only: true, reversible: true, parallel_safe: true,
    requires_confirmation: false, requires_network: false, requires_filesystem: true,
    timeout: 15, cost: C(50, 150, 2000, { cpu: "low" }), verification_required: false, idempotent: true,
    preferred_for: ["choose between plan shapes before committing to one", "check whether adding a verification step raises expected verified progress", "surface single points of failure before execution"],
    avoid_when: ["executing the plan (it never executes anything)", "plans larger than 24 nodes"],
    mutates: [],
  },
  {
    name: "code_context",
    description: "One-call context pack: semantic (meaning) hits over chunked sources plus the structural wiring of the top files (importers, covering tests, blast radius).",
    capabilities: [CAPABILITY.CONTENT_SEARCH, CAPABILITY.SYMBOL_LOOKUP, CAPABILITY.FILE_DISCOVERY],
    klass: CLASS.READ, classes: [CLASS.READ],
    risk: RISK.LOW, read_only: true, reversible: true, parallel_safe: true,
    requires_confirmation: false, requires_network: false, requires_filesystem: true,
    timeout: 30, cost: C(300, 900, 12000, { cpu: "medium" }), verification_required: false, idempotent: true,
    preferred_for: ["understand how an area is wired before the first edit", "combine meaning search with structural facts in one call", "verifiers gathering context without mutating anything"],
    avoid_when: ["the exact symbol/text is known (grep_files is cheaper)", "only the meaning search is needed (semantic_search is lighter)"],
    mutates: [],
  },
]
