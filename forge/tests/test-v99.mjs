...
console.log("== 6. reach: mcpcatalog + skillregistry ==")
{
  ...
  ok("catalog is non-empty and frozen", mc.MCP_CATALOG.length >= 10 && Object.isFrozen(mc.MCP_CATALOG))
  const names = new Set(mc.MCP_CATALOG.map((e) => e.name))
  ok("catalog names unique", names.size === mc.MCP_CATALOG.length)
  ok("well-known presets present", ["filesystem", "playwright", "github", "context7"].every((n) => names.has(n)))
  ok("every entry has command+args+desc+homepage", mc.MCP_CATALOG.every((e) => e.command && Array.isArray(e.args) && e.desc && e.homepage))
  ...
  eq("github spec command", spec.command, "npx")
  ok("github spec requires its env var", spec.env && "GITHUB_PERSONAL_ACCESS_TOKEN" in spec.env)
  eq("env placeholder is EMPTY — secrets never invented", spec.env.GITHUB_PERSONAL_ACCESS_TOKEN, "")
  const inst = mc.envInstructions(gh, "github")
  ok("env instructions name the exact config key", inst.length === 1 && inst[0].includes("mcp.servers.github.env.GITHUB_PERSONAL_ACCESS_TOKEN"))
  ok("no-env entries get no env block", mc.specForEntry(mc.catalogEntry("memory")).env === undefined)
