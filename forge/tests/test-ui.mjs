...
const { renderBlock, renderLine, createMarkdownStream } = await import("../markdown.js")
const { createKeyDecoder } = await import("../keys.js")
const { createEditor, layout, parseHistoryFile, serializeHistory, historyWorthy, dedupe } = await import("../editor.js")
const { createUIStore, reduce, initialState, bridgeAgentEvent, createBridgeContext, classifyCheck, parseCheckOutput, parseTodo, summarizeToolResult, toolTarget, isBusy } = await import("../uistate.js")
const { createTerminal } = await import("../terminal.js")
const { openRun, interruptedRuns, verifyRun, listRuns, markRun, pruneRuns, RUNS_DIR, resolveRunId } = await import("../runlog.js")
const { unifiedDiff, diffStats } = await import("../textdiff.js")
const { applyUnifiedDiff } = await import("../diffpatch.js")
const { createScreen } = await import("./vtscreen.mjs")
const { suggestCommand, COMMANDS } = await import("../chat.js")
...
`
  ...
  // v96 unifywise: this pin was STALE since v95 — the PTY lane is skipped under
  // FORGE_FAST=1, so "forge v94" survived two version bumps unnoticed. The
  // banner prints the real VERSION (ui.printBanner); the pin now derives from
  // version.js instead of a hardcoded string, so a future bump cannot make
  // this silently stale again.
  const { VERSION: PTY_VERSION } = await import("../version.js")
  ok("pty: banner + answer streamed", r1.text.includes(`forge v${PTY_VERSION}`) && r1.text.includes("Hello from mock!"))
