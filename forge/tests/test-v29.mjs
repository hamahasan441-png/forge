...
const { informationGain, selectExperiment, rankExperiments, createInfoGainEngine, formatExperiment, XID, XKIND } = await import("../infogain.js")
const { BENCH_CASES, runBench, runCase, formatReport } = await import("../bench.js")
const { createKernel } = await import("../omega.js")
const { FAILURE } = await import("../diagnose.js")
const { ORIGIN } = await import("../selfdiag.js")
const { TASK_CLASS, classifyTaskComplexity, strategyFor } = await import("../classify.js")
...
console.log("== FORGE-BENCH 16/16, no live model ==")
{
  eq("16 cases", BENCH_CASES.length, 16)
  const summary = runBench()
  eq("bench total 16", summary.total, 16)
...
console.log("== safety + wiring (source) ==")
{
  ...
  const omega = fs.readFileSync(new URL("../omega.js", import.meta.url), "utf8")
...
{
  eq("VERSION is 91.0.0", VERSION, "91.0.0")
  eq("package.json is 91.0.0", JSON.parse(fs.readFileSync(new URL("../package.json", import.meta.url), "utf8")).version, "91.0.0")
}
