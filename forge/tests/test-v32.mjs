...
const { buildRepoMap, getIndexStats, buildSemanticGraph } = await import("../repomap.js")
...
{
  eq("INDEX_VERSION", INDEX_VERSION, 2) // v98 shipwise: one-time re-extraction bump (provenance fields + JSON tier)
  ...
  eq("empty version", empty.version, 2)
  ...
  ok("corrupt → empty, no throw", loaded.version === 2 && Object.keys(loaded.files).length === 0)
