...
import { classifyCommand, modelMayRun, userMayRun, splitSubcommands, tokenize, LEVELS, FORBIDDEN as SHELL_FORBIDDEN } from "../shellguard.js"
import { execTool, safePath, validSkillName, TOOL_DEFS, WRITE_TOOLS, selfTestTools, FORBIDDEN as TOOLS_FORBIDDEN } from "../tools.js"
import { redact, redactSecrets } from "../secrets.js"
import { isPrivateAddress, assertFetchableUrl, BLOCKED_HOSTNAMES } from "../netguard.js"
import { snapshotBefore, sealCreated, restoreLast, listCheckpoints, CHECKPOINTS_DIR } from "../checkpoint.js"
import { appendMemory, recordLearning, relevantMemory, relevantLearnings, memoryStats, GLOBAL_MEMORY_PATH, PROJECTS_DIR, projectHash, projectDir, projectMemoryPath, memoryPathFor } from "../memory.js"
import { loadProfile, profileSummary, resourceProfile } from "../profile.js"
import { saveSession, loadSession, listSessions, findSession, lastSessionFile } from "../sessions.js"
import { loadSkill, validSkillName as validSkillName2 } from "../skills.js"
import { ProviderError } from "../providers.js"
import { agentEventPrinter } from "../agent.js"
import { bold, dim, cyan, green, yellow, red, magenta, info, ok as uiOk, warn, err, renderMarkdown, printBanner, estimateTokens } from "../ui.js"
import { wizardProviderSteps, runConfigMenu } from "../onboard.js"
...
console.log("== shellguard: policies (v88 noguard — verdicts always ok) ==")
eq("model may run npm test", modelMayRun("npm test", { cwd: WORK }).ok, true)
eq("v88: rm -rf / allowed for the model (level kept)", modelMayRun("rm -rf /", { cwd: WORK }).ok, true)
eq("v88: outside rm allowed for the model", modelMayRun(`rm -rf ${OUT}`, { cwd: WORK, root: WORK }).ok, true)
eq("model may rm inside project", modelMayRun("rm -rf node_modules", { cwd: WORK, root: WORK }).ok, true)
eq("v88: sudo allowed for the model without config", modelMayRun("sudo ls", { cwd: WORK }).ok, true)
eq("model sudo allowed when configured", modelMayRun("sudo ls", { cwd: WORK }, { allowSudo: true }).ok, true)
eq("v88: block-class allowed for the user", userMayRun("mkfs /dev/sda", { cwd: WORK }).ok, true)
eq("v88: piped confirm runs without ASSUME_YES", userMayRun("rm file.txt", { cwd: WORK }, { interactive: false }).ok, true)
eq("user piped assumeYes passes", userMayRun("rm file.txt", { cwd: WORK }, { interactive: false, assumeYes: true }).ok, true)
eq("v88: no confirm prompt on a TTY", userMayRun("rm file.txt", { cwd: WORK }, { interactive: true }).needsConfirm, false)

console.log("== safePath: v88 noguard (no boundary, no sensitive-read block) ==")
const sp1 = safePath(ctx, "../../outside.txt", { write: true })
ok("v88: write outside root ALLOWED", sp1.ok)
...
ok("v88: read id_rsa allowed (no guard)", sp4.ok)
const sp5 = safePath({ ...ctx, cwd: "/tmp", root: "/tmp" }, "x/../../.env")
ok("v88: read .env via traversal allowed", sp5.ok)
const sp6 = safePath({ ...ctx, cwd: WORK, root: WORK }, path.join(process.env.FORGE_HOME, "config.json"))
ok("v88: read forge config allowed", sp6.ok)
const sp7 = safePath({ ...ctx, cwd: "/tmp", root: "/tmp" }, "/etc/shadow")
ok("v88: read /etc/shadow allowed by policy (OS perms still apply)", sp7.ok)
...
ok("v88: symlink write escape allowed (no boundary)", sp8.ok)
const sp9 = safePath({ ...ctx, cwd: "/tmp", root: "/tmp" }, "/etc/hosts")
ok("read /etc/hosts allowed (world-readable, not secret)", sp9.ok)

console.log("== safePath via tools (v88 noguard: verdicts checked, nothing destructive executed) ==")
ok("v88: outside-project rm verdict is ok", modelMayRun(`rm -rf ${OUT}`, { cwd: WORK, root: WORK }).ok === true)
const b2 = await execTool(ctx, "bash", { command: "echo tool-ok" })
ok("bash tool runs safe commands", String(b2).includes("tool-ok"))
ok("v88: root-wipe verdict is ok (never executed in a test)", modelMayRun("rm -rf /", { cwd: WORK, root: WORK }).ok === true && modelMayRun("rm -rf /", { cwd: WORK, root: WORK }).level === "block")
ok("v88: sudo verdict is ok without config", modelMayRun("sudo ls /root", { cwd: WORK, root: WORK }).ok === true)
const w1 = await execTool(ctx, "write_file", { path: "../../escaped.txt", content: "x" })
ok("v88: write_file outside root ALLOWED", String(w1).includes("OK wrote"))
...
ok("v88 noguard: fetch_url allows loopback (no SSRF gate)", !String(f1).startsWith("BLOCKED"))
const fctx = {
...
ok("v88: model allowed mv into /etc (classified block, never refused)", modelMayRun("mv file.txt /etc", { cwd: WORK, root: WORK }).ok === true)
ok("v88: model allowed cp into /etc (classified block, never refused)", modelMayRun("cp file.txt /etc", { cwd: WORK, root: WORK }).ok === true)
ok("v88: user allowed mv into /etc", userMayRun("mv file.txt /etc", { cwd: WORK }, { interactive: true }).ok === true)
...
ok("v88 noguard: failed-closed command still returns ok for the model (no refusal path left)", modelMayRun(boom, { cwd: WORK, root: WORK }).ok === true)
ok("v88 noguard: failed-closed command returns ok for the user", userMayRun(boom, { cwd: WORK }, { interactive: false }).ok === true)
...
ok("v88: sh -c root wipe allowed (level kept)", modelMayRun('sh -c "rm -rf /"', { cwd: WORK, root: WORK }).ok === true)
ok("v88: python os.system root wipe allowed (level kept)", modelMayRun('python3 -c "import os; os.system(\'rm -rf /\')"', { cwd: WORK, root: WORK }).ok === true)
ok("v88: xargs rm allowed", modelMayRun("xargs rm -rf /tmp/x", { cwd: WORK, root: WORK }).ok === true)
ok("v88: python -c allowed without consent", modelMayRun('python3 -c "print(1)"', { cwd: WORK, root: WORK }).ok === true)
...
ok("TOOL_DEFS defines all 30 tools (v94c toolwise)", Array.isArray(TOOL_DEFS) && TOOL_DEFS.length === 30)
