...
{
  eq("a failing test suite", L.classifyLessonFailure("npm test failed: 3 tests failed in auth.spec.ts"), "test_failure")
  eq("a missing module", L.classifyLessonFailure("Error: Cannot find module 'crypto'"), "dependency_failure")
  eq("a segfault", L.classifyLessonFailure("segmentation fault (core dumped)"), L.FAILURE_CLASS.TOOL_CRASH)
  eq("an OOM kill", L.classifyLessonFailure("JavaScript heap out of memory"), L.FAILURE_CLASS.TOOL_CRASH)
  eq("a compile error", L.classifyLessonFailure("src/a.ts(4,7): error TS2322"), "build_failure")
  eq("a timeout", L.classifyLessonFailure("command timed out after 120000ms"), L.FAILURE_CLASS.TOOL_TIMEOUT)
  eq("a permission error", L.classifyLessonFailure("EACCES: permission denied"), L.FAILURE_CLASS.SECURITY_BLOCK)
  eq("a tool misuse", L.classifyLessonFailure("arguments did not match the schema for edit_file"), L.FAILURE_CLASS.TOOL_MISUSE)
  eq("an unclassifiable failure falls back", L.classifyLessonFailure("the widget was malformed"), L.FAILURE_CLASS.UNKNOWN)
}
