...
console.log("== deepwise: critique wiring in the tool-intel pipeline ==")
{
  ...
  ok("edit on a missing target is BLOCKED before exec (v112 criticwise)", /^BLOCKED: \(critique\) BLOCK/.test(String(res.result)), String(res.result).slice(0, 180))
  ok("critique why names the missing file", /ghost\.js does not exist/.test(String(res.result)), String(res.result))
  ...
  ok("blocked result is not a fake journal write", !/created|deleted/i.test(String(res.result)))
