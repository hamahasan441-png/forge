...
const { recordProjectStrategy, pickStrategy, loadStrategies } = await import("../strategy.js")
...
{
  recordProjectStrategy({ cwd: WORK, name: "playbook-first", ok: true })
  recordProjectStrategy({ cwd: WORK, name: "playbook-first", ok: true })
  recordProjectStrategy({ cwd: WORK, name: "rediscover", ok: false })
...
console.log("== gap tests never invent; steer STRAT; bench 2.0 ==")
{
  ...
  ok("bench 20 cases", BENCH_CASES.length >= 20, BENCH_CASES.length)
  ...
  ok("compose has no recordProjectStrategy", !/recordProjectStrategy/.test(composeSrc))
