...
} = await import("../langengine.js")
const { makeToolContext, safePath } = await import("../tools.js")
const { verificationPlan, VERIFY_CHECK } = await import("../verify.js")
...
console.log("== verificationPlan HIGH TESTS uses recommendedVerify ==")
{
  ...
  const rustTests = rust.checks.find((c) => c.kind === VERIFY_CHECK.TESTS)
  ...
  eq("HIGH js command is npm test", js.checks.find((c) => c.kind === VERIFY_CHECK.TESTS)?.command, "npm test")
  const empty = verificationPlan("write_file", { path: "notes.txt" }, { risk: RISK.HIGH, cwd: WORK, meta })
  const emptyTests = empty.checks.find((c) => c.kind === VERIFY_CHECK.TESTS)
