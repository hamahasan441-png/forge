...
- Dashboard runtime: the playbook's `packages/dashboard` lookup will not find
  a plugin root inside forge. Use the playbook's FAST PATH instead — the
  version-pinned prebuilt viewer tarball from the upstream releases
  (`npx --yes
  "https://github.com/Egonex-AI/Understand-Anything/releases/download/v<version>/understand-anything-viewer.tgz" "$PROJECT_DIR"`,
  version read from the upstream repo's package.json), which needs no plugin
  root and no build. Only if the user explicitly wants the dev dashboard,
  clone the upstream repo with bash
  (`git clone --depth 1 https://github.com/Egonex-AI/Understand-Anything`),
  then `npm install` + `npm run dev` inside `packages/dashboard` — surface
  the local URL forge's runtime health tools report.
- Graph prerequisite: `.ua/knowledge-graph.json` must exist — run the
  `understand` skill first when it does not.
