...
## Forge execution notes (forge agent)

This skill is running inside **forge**, which has no Claude/Codex plugin
runtime. The playbook above is the authority; adapt ONLY the platform seams:

- `SKILL_DIR` — the directory containing THIS SKILL.md. Forge's `load_skill`
  output begins with a `[skill dir: <absolute path>]` line: use that exact
  path everywhere the playbook writes `<SKILL_DIR>`. Never guess a plugin
  root or a `~/.agents` / `~/.claude` path — those installs do not exist here.
- Subagent definitions — the playbook's `agents/<name>.md` files are bundled
  INSIDE this skill directory (`<SKILL_DIR>/agents/`). `read_file` the
  definition first, then either assume the role inline or hand the definition
  text (plus the appended task context the playbook specifies) to the
  `delegate` tool with a matching role. Run up to 5 delegates concurrently
  where the playbook says so — forge caps delegate concurrency itself.
- Helper scripts — every `node "<SKILL_DIR>/…"` / `python3 "<SKILL_DIR>/…"`
  command in the playbook runs as-is through forge's bash tool, from the
  analyzed project's root. The bundled scripts are self-contained (Node
  built-ins / Python 3 stdlib) — no npm install, no pip install.
- Data directory — all generated data stays in the analyzed project's
  `.ua/` directory (`.ua/knowledge-graph.json` is the graph).

- Bundled assets in this skill directory: `scan-project.mjs`,
  `extract-structure.mjs`, `extract-import-map.mjs`, `compute-batches.mjs`,
  `build-fingerprints.mjs`, `prepare-incremental.mjs`,
  `finalize-incremental.mjs`, `prepare-symbol-retry.mjs`,
  `validate-incremental-symbols.mjs`, `extract-structure-result.mjs`,
  `generate-ignore.mjs`, `merge-batch-graphs.py`,
  `merge-subdomain-graphs.py`, plus `frameworks/`, `languages/` and
  `locales/` reference data — all paths the playbook names relative to
  `<SKILL_DIR>` resolve here.
- Subagent definitions bundled: `agents/project-scanner.md`,
  `agents/file-analyzer.md`, `agents/assemble-reviewer.md`,
  `agents/architecture-analyzer.md`, `agents/tour-builder.md`,
  `agents/graph-reviewer.md` (the `--review` reviewer path).
- Forge synergy: once `.ua/knowledge-graph.json` exists, forge's engineering
  memory retrieves its architecture facts (framework, layers, hub modules)
  automatically on later tasks in this project.
